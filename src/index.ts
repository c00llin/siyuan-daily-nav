...
import { openTab, sql, createDocWithMd, getNotebookConf, renderSprig, lsNotebooks, setBlockAttrs } from "./api";
...
export default class DailyNavPlugin extends Plugin {
    ...
    async openSetting() {
        ...
        const container = dialog.element.querySelector("#daily-nav-settings") as HTMLElement;
        if (!container) return;
    ...
    async loadNotebooksToSelect() {
        const select = document.getElementById("fallback-notebook-select") as HTMLSelectElement;
        if (!select) return;
    ...
    async navigateToDailyNote(protyle: any, dayOffset: number) {
        try {
            ...
            if (window.siyuan?.ws?.app?.plugins) {
                const layout = document.querySelector('.layout__wnd--active .protyle.fn__flex-1:not(.fn__none)');
                if (layout) {
            ...
            if (docId) {
                ...
                if (newDocId) {
                    ...
                    showMessage(`Failed to create daily note: ${targetDateStr}`, -1, "error");
                }
            }
        } catch (error) {
     ...
     */
    async extractDateFromDocument(docId: string): Promise<{ date: Date; notebookId: string } | null> {
        try {
            ...
            if (result && result.length > 0) {
                ...
                const dateMatch = (doc.content || doc.hpath).match(/(\d{4})-(\d{2})-(\d{2})/);
     ...
     */
    async createDailyNote(dateStr: string, preferredNotebookId?: string | null): Promise<string | null> {
        try {
            ...
            if (!notebookId) {
                ...
                if (this.settings.fallbackNotebook) {
                    ...
                    if (notebooks && notebooks.notebooks && notebooks.notebooks.length > 0) {
                        ...
                        const openNotebook = notebooks.notebooks.find((nb: any) => !nb.closed);
                        if (openNotebook) {
            ...
            if (!notebookId) {
                throw new Error("Could not determine notebook ID. Please configure a fallback notebook in settings.");
            }
            ...
            let dailyNotePath = notebookConf.conf?.dailyNoteSavePath || "/{{now | date \"2006-01-02\"}}/";
